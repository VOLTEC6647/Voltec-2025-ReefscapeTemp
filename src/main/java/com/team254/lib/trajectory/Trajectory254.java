...
public class Trajectory254<S extends State<S>> implements CSVWritable {
     ...
     */
    public Trajectory254(final List<S> states) {
    ...
    public class IndexView implements TrajectoryView<S> {
        ...
        public TrajectorySamplePoint<S> sample(double index) {
            return Trajectory254.this.getInterpolated(index);
        }

        @Override
        public double last_interpolant() {
            return Math.max(0.0, Trajectory254.this.length() - 1);
        }
        ...
        @Override
        public Trajectory254<S> trajectory() {
            return Trajectory254.this;
        }
    }
}
