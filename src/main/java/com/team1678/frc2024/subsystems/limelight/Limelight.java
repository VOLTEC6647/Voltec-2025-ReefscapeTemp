...
public class Limelight extends Subsystem {
	...
	private GoalTracker noteTracker = new GoalTracker(Constants1678.LimelightConstants.kNoteTrackerConstants);
			...
			List<Translation2d> targetPositions = Arrays.stream(inputs.results.targetingResults.targets_Detector)
					.map(target -> {
						...
						Translation2d targetPosition =
								getCameraToGoalTranslation(targetInfo, Constants1678.LimelightConstants.kNoteHeight);
	...
	public NotePosition matchNotePosition(Translation2d detected_position) {
		double lowest_error = Constants1678.LimelightConstants.kNoteTrackEpsilon;
	...
	public boolean hasNote(NotePosition note) {
		return noteScan().contains(note)
				&& Drive.getInstance()
								...
								.norm()
						<= Constants1678.LimelightConstants.kMaxNoteTrackingDistance;
	}
