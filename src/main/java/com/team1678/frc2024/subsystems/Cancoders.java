...
public class Cancoders {
	...
	private Cancoders() {
		mFrontLeft = build(Ports1678.FL_CANCODER);
		mFrontLeftObserver = new CanTsObserver(mFrontLeft);

		mFrontRight = build(Ports1678.FR_CANCODER);
		mFrontRightObserver = new CanTsObserver(mFrontRight);

		mBackLeft = build(Ports1678.BL_CANCODER);
		mBackLeftObserver = new CanTsObserver(mBackLeft);

		mBackRight = build(Ports1678.BR_CANCODER);
