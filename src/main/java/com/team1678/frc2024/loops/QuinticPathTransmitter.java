...
public class QuinticPathTransmitter implements Loop {
	...
	public void addPath(Trajectory254<TimedState<Pose2dWithMotion>> path) {
		remainingTrajectories.add(new TrajectoryIterator<>(new TimedView<>(path)));
		currentTrajectory = null;
	}

	public void addPaths(List<Trajectory254<TimedState<Pose2dWithMotion>>> paths) {
	...
	public double getTotalPathTime(List<Trajectory254<TimedState<Pose2dWithMotion>>> paths) {
		double total = 0.0;

		for (Trajectory254<TimedState<Pose2dWithMotion>> path : paths) {
