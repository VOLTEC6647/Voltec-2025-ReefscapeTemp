...
public class SwerveModule extends Subsystem {
	...
	public SwerveModule(int moduleNumber, SwerveModuleConstants moduleConstants, CANcoder cancoder) {
		...
		// Angle motor config
		mAngleMotor = new TalonFX(moduleConstants.angleMotorID, DriveConstants.swerveCANBus);
		Phoenix6Util.checkErrorAndRetry(() ->
				mAngleMotor.getConfigurator().apply(SwerveConstants.AzimuthFXConfig(), Constants1678.kLongCANTimeoutMs));
		mAngleMotor.setInverted(SwerveConstants.angleMotorInvert);

		// Drive motor config
		mDriveMotor = new TalonFX(moduleConstants.driveMotorID, DriveConstants.swerveCANBus);
		Phoenix6Util.checkErrorAndRetry(() ->
				mDriveMotor.getConfigurator().apply(SwerveConstants.DriveFXConfig(), Constants1678.kLongCANTimeoutMs));
	...
	public synchronized void refreshSignals() {
		mPeriodicIO.rotationVelocity = mAngleMotor.getRotorVelocity().getValue().baseUnitMagnitude();
		mPeriodicIO.driveVelocity = mDriveMotor.getRotorVelocity().getValue().baseUnitMagnitude();
	...
	public void setVelocity(SwerveModuleState desiredState) {
		...
		double rotorSpeed = Conversions.MPSToRPS(
				...
				Constants1678.SwerveConstants.wheelCircumference,
	...
	private void setSteeringAngleRaw(double angleDegrees) {
		...
		//new PositionDutyCycle(rotorPosition, 0.0, true, 0.0, 0, false, false, false);
	}
	...
	public Rotation2d getCanCoder() {
		return Rotation2d.fromDegrees(angleEncoder.getAbsolutePosition().getValue().baseUnitMagnitude() * 360.0);
	}
	...
	public double getDriveMotorCurrent() {
		return mDriveMotor.getStatorCurrent().getValue().baseUnitMagnitude();
	}
